...
import dev.pet.pets.service.PetSearchService;
...
public class PetController {
    ...
    private final PetSearchService searchService;
    private final DigestionService digestionService;

    public PetController(PetService service, PetSearchService searchService, DigestionService digestionService) {
    ...
    public Page<PetListItemResponse> searchMyPets(
        ...
        @RequestParam(required = false) List<String> sizeCategories,
        ...
        @RequestParam(required = false) List<String> healthConditionCodes,
        @RequestParam(required = false) String favorite,
        @RequestParam(required = false) String recipeStatus,
        @RequestParam(defaultValue = "0") int page,
        ...
        filter.setAgeGroups(ageGroups);
        filter.setMinWeight(minWeight);
        filter.setMaxWeight(maxWeight);
        filter.setBreedIds(breedIds);
        filter.setSizeCategories(sizeCategories);
        filter.setReproductiveStatusIds(reproductiveStatusIds);
        filter.setActivityTypeIds(activityTypeIds);
        filter.setSymptomIds(symptomIds);
        filter.setHealthConditionCodes(healthConditionCodes);
        filter.setFavorite(favorite);
        filter.setRecipeStatus(recipeStatus);
        return searchService.searchMine(jwt, filter, PageRequest.of(page, size));
    }

    @PostMapping("/{id}/favorite")
    public void addFavorite(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID id) {
        searchService.addFavorite(jwt, id);
    }

    @DeleteMapping("/{id}/favorite")
    public void removeFavorite(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID id) {
