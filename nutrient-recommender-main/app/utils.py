...
def select_missing_ingredients(deficit_info, ingredients_df):

    allowed_categories = [ "Крупы", "Овощи и фрукты", "Масло и жир","Зелень и специи"]
    candidates = ingredients_df[ingredients_df["category_ru"].isin(allowed_categories)].copy()

    remaining_deficit = {nutr: {"deficit": info["deficit"],
            "deficit_percent": info["deficit_percent"],
            "deficit_per_100g": info["deficit_per_100g"]
        } for nutr, info in deficit_info.items() }
    ...
    while True:
        ...
        available_deficits = {
            nutr: info["deficit_percent"]
            for nutr, info in remaining_deficit.items()
            if (info["deficit"] > 0 and info["deficit_percent"] > 10 and nutr not in processed_nutrients  ) }

        if not available_deficits:
            break

        target_nutrient = max(available_deficits,key=available_deficits.get)
        target_deficit = remaining_deficit[target_nutrient]["deficit"]
        target_percent = remaining_deficit[target_nutrient]["deficit_percent"]

        if target_nutrient not in candidates.columns:
            processed_nutrients.add(target_nutrient)
            continue

        nutrient_candidates = candidates[ candidates[target_nutrient].notna() & (candidates[target_nutrient] > 0)].copy()

        if nutrient_candidates.empty:
            processed_nutrients.add(target_nutrient)
            continue

        best_idx = nutrient_candidates[target_nutrient].idxmax()
        ingredient = nutrient_candidates.loc[best_idx]
        ingredient_name = ingredient["ingredient_format_cat"]

        nutrient_per_100g = float(ingredient[target_nutrient])
        exact_amount = (target_deficit/ (nutrient_per_100g / 100))
        amount = float(np.clip(exact_amount, 1, 10))

        nutrient_changes = {}

        for nutr in remaining_deficit:

            if nutr not in ingredient.index:
                continue

            value = ingredient[nutr]

            if pd.isna(value):
                continue

            value = float(value)
            added = value * amount / 100
            nutrient_changes[nutr] = added

        for nutr, added in nutrient_changes.items():

            old_deficit = remaining_deficit[nutr]["deficit"]
            new_deficit = max(old_deficit - added, 0)
            remaining_deficit[nutr]["deficit"] = new_deficit
            old_percent = remaining_deficit[nutr]["deficit_percent"]

            if old_percent > 0:
                original_deficit = (deficit_info[nutr]["deficit"])
                original_percent = (deficit_info[nutr]["deficit_percent"])

                if original_deficit > 0:
                    new_percent = (new_deficit / original_deficit * original_percent)
                else:
                    new_percent = 0
            else:
                new_percent = 0

            remaining_deficit[nutr]["deficit_percent"] = new_percent
            original_per_100g = (deficit_info[nutr]["deficit_per_100g"])
            original_deficit = (deficit_info[nutr]["deficit"])

            if original_deficit > 0:
                remaining_deficit[nutr]["deficit_per_100g"] = (new_deficit/ original_deficit * original_per_100g)
            else:
                remaining_deficit[nutr]["deficit_per_100g"] = 0

        selected_ingredients.append({
            "ingredient": ingredient_name,
            "amount_g_per_100g": amount
        })
