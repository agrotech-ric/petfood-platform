...
- [ ] 5.3 Merge the archived final legacy `main` as a history-only parent, then verify the resulting source tree against the approved beta-derived candidate with tree hashes, a full diff, and an explicit check that legacy runtime files were not reintroduced.
