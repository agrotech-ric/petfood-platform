...
- Permit the exact final legacy generation to return as an explicitly accepted
  emergency rollback even though its historical session and photo-security
  behavior does not satisfy the promoted beta security baseline; keep the
  limitation visible in the release record and use maintenance mode during the
  switch.
...
- Active application code remains within `frontend-next`, `backend-main-sandbox`, `nutrient-recommender-main`, and the beta runtime definition. Legacy `frontend-main`, `backend-main`, and main compose files are preserved as the coordinated historical rollback generation and are not combined into the promoted runtime.
...
- Rollback restores the final legacy code/runtime and its matching data as one coordinated generation; partial code-only or data-only rollback is not permitted. The owner explicitly accepts the documented historical security limitations if this emergency generation must be reopened.
